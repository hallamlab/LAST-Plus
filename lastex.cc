...
#undef NEW
#ifdef NEW
    void bypass( int argc, char** argv );
#endif
...
void writeEvalues( const std::string& matrixString ){
  ...
  __makeScoreMatrix( matrixString );
  makeStrandStats();
  makeEvaluer();
...
void parseHeader( const std::vector<std::string>& lines,
  ...
  if( headerMatrix.empty() || a < 0 || b < 0 || j < 0 || s < 0 ){
    ...
    if( args.matchScore < 0 && args.mismatchCost < 0 &&
      ...
      args.isGapless = (j < 2);
      args.strand = s;
      __makeScoreMatrix( headerMatrix );
...
void parseChunk( const std::vector<std::string>& lines,
  ...
  if( evaluer.isBad() ) { 
     ...
     parseHeader( lines, matrixString );
   }
...
void bypass( int argc, char** argv ){
  ...
  fastaFileSequenceStats(targetStatsFile, &stats1);
  fastaFileSequenceStats(queryStatsFile, &stats2);
