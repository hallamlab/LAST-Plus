...
void Alignment::write(
		...
		const Alphabet& alph, int format, 
		std::list<std::string> *outputList, const AlignmentExtras& extras ) const{

	assert( !blocks.empty() );

	if( format == 0 ) {
		writeTab( reference, query, strand, isTranslated, extras, outputList );
	} else if( format == 2 )  {
		writeBlastOutput(scoreCutoff, evalueCutoff, reference, query, strand,
				isTranslated, alph, extras, outputList);
	} else {
		writeMaf( reference, query, strand, isTranslated, alph, extras ,outputList);
	}
}
...
void Alignment::writeBlastOutput(
		...
		const MultiSequence& reference, const MultiSequence& query,
		char strand, bool isTranslated, 
		const Alphabet& alph, const AlignmentExtras& extras, 
	...
	if(bitscore >= scoreCutoff && bit_evalue <= evalueCutoff){
		...
		outputList->push_back( outputStream.str() );
	}
}
